...
public class ThirdPersonOrbitCamBasicMobile : MonoBehaviour
{
    ...
    [SerializeField] private float rightThreshold; // The x-coordinate threshold for 75% of the screen width
    ...
    private void Start()
    {
        ...
        rightThreshold = screenWidth * 0.35f;
    }


    // Detect if it's a camera move or an analog drag.
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0)
        {
            ...
            if (!touchFound)
            {
                for (int i = 0; i < Input.touchCount; i++)
                {
                    if (Input.GetTouch(i).position.x > rightThreshold)
                    {
            ...
            if (touchFound)
            {
                // Ensure the touch is happening on the right side (75%) of the screen
                if (touch.position.x > rightThreshold)
                {
                    ...
                    if (camTouchscreen && touchID >= 0)
                    {
                        angleH += (Input.GetTouch(touchID).deltaPosition.x / 10f) * horizontalAimingSpeed * Time.deltaTime;
                        angleV += (Input.GetTouch(touchID).deltaPosition.y / 10f) * verticalAimingSpeed * Time.deltaTime;
                    }
                }
            }
        }
...
#else
        Cursor.visible = false;
        // Get axes movement to orbit the camera using the mouse.
        angleH += Input.GetAxis("Mouse X") * 60 * horizontalAimingSpeed * Time.deltaTime;
        angleV += Input.GetAxis("Mouse Y") * 60 * verticalAimingSpeed * Time.deltaTime;
#endif
        Cursor.visible = false;
        ...
        // Test for collision with the environment based on the current camera position.
    ...
    // Double check for collisions: concave objects don't detect a hit from outside, so cast in both directions.
