...
namespace EasyBuildSystem.Examples.Bases.Scripts.UI.Mobile
{
    public class Demo_UIAndroidControls : MonoBehaviour
    {
        ...
        [SerializeField] Inventory1 playerInventory;

        public void VirtualValidateBuildingAction()
        {
            if(BuildingPlacer.Instance.GetSelectedBuildingPart.gameObject.GetComponent<BuildingMaterials>() != null) {
                if(BuildingPlacer.Instance.GetSelectedBuildingPart.gameObject.GetComponent<BuildingMaterials>().UseMaterials(playerInventory)) {
                    Debug.Log("WE HAVE MATERIALS TO BUILD!");
                    m_AndroidInputHandler.HandleBuildModes();
                }
                else
                {
                    Debug.Log("We don't have materials");
                }
            }
            else
            {
                Debug.Log("WE CAN BUILD, NO MATERIALS NEEDED!");
                m_AndroidInputHandler.HandleBuildModes();
            }
        }
